...
websocket_urlpatterns = [
    ...
    re_path(r"manager/sung/(?P<store_id>\d+)/(?P<room_name>\w+)/$", manager_consumers.ManagerConsumer.as_asgi()),
    ...
    re_path(r"manager/sung/test/(?P<store_id>\d+)/(?P<room_name>\w+)/$", manager_channels.ManagerConsumer.as_asgi()),
    # 테스트용
    re_path(r"manager/sung/update/(?P<store_id>\d+)/(?P<room_name>\w+)/$", test_consumers.TestConsumer.as_asgi())
